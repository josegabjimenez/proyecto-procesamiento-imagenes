...
def count_volume(image_path):
    image_flair = nib.load(os.path.join("images/1", "FLAIR.nii.gz"))

    image = nib.load(os.path.join("temp_images", image_path))
...
def registration(
    fixed_image_path, moving_image_path, segmentated_image_path, type="Rigid"
):
    ...
    if segmentated_image_path != "":
        moving_image_segmented = ants.image_read(
            os.path.join("temp_images", segmentated_image_path)
        )
    else:
    ...
    transform = ants.registration(
        fixed=fixed_image, moving=moving_image, type_of_transform=type
    )
